...
export const MultiSelectWithReorder: Story = {
  render: () => {
    ...
    const updateOrder = (moveIds: string[], targetId: string) => {
        ...
        <TableBody>
          {table.getRowModel().rows.map(row => (
            <ReorderRow
              ...
              row={row}
              id={row.original.id}
              updateOrder={(moveId, targetId) => {
                const selectedRows = table.getSelectedRowModel().flatRows.map(r => r.original.id);
                const rowsToMove = selectedRows.length > 1 ? selectedRows : [moveId];
                updateOrder(rowsToMove, targetId);
              }}
