...
import type { ViewUpdate, PluginValue, EditorView, Tooltip } from '@codemirror/view';
import type { Text } from '@codemirror/state';
import type { Transport } from '@open-rpc/client-js/build/transports/Transport';
import { fromMarkupContent } from './markdownToDom';

const timeout = 10000;
const changesDelay = 500;

const CompletionItemKindMap = Object.fromEntries(Object.entries(CompletionItemKind).map(([key, value]) => [value, key])) as Record<
  CompletionItemKind,
  string
>;
...
class LanguageServerPlugin implements PluginValue {
  ...
  async requestHoverTooltip(view: EditorView, { line, character }: { line: number; character: number }): Promise<Tooltip | null> {
    ...
    return { pos, end, create: () => ({ dom: fromMarkupContent(contents, document.createElement('div')) }), above: true };
  }
  ...
  async requestCompletion(
    ...
    let options = items.map(({ detail, label, kind, insertText, textEdit, documentation, sortText, filterText }) => {
      const completion: Completion & {
        ...
        label,
        detail,
        apply: insertText ?? textEdit?.newText ?? label,
        type: kind && CompletionItemKindMap[kind].toLowerCase(),
        sortText: sortText ?? label,
        filterText: filterText ?? label
      };
      if (documentation) {
        completion.info = () => fromMarkupContent(documentation, document.createElement('div')); //formatContents(documentation);
      }
      return completion;
    });
...
const compareCompletions = (a: Completion, b: Completion) => {
  //@ts-ignore
  const compare = a.sortText.localeCompare(b.sortText);
  if (compare === 0) {
